...
/*
  int temp;
  temp= __raw_readl(S3C_GPBCON);
  temp&=0x3fffff;               0011,GPE15 INPUT 
  __raw_writel(temp,S3C_GPBCON);
*/
  s3c_gpio_cfgpin(S3C_GPB6,S3C_GPB6_INP);  
}
...
/*  
  int temp;
  temp= __raw_readl(S3C_GPBCON);
  temp=((temp&0x3fffff)|1<<24); 0011|1<<30,GPE15 OUTPUT
  __raw_writel(temp,S3C_GPBCON);
  temp= __raw_readl(S3C_GPBCON);
*/
  s3c_gpio_cfgpin(S3C_GPB6,S3C_GPB6_OUTP);      
}
...
/* 
   int temp;
   temp= __raw_readl(S3C_GPBCON);
   temp=((temp&0xcfffff)|1<<20);1100|1<<28,GPE14 output
   __raw_writel(temp,S3C_GPBCON);
*/
   s3c_gpio_cfgpin(S3C_GPB5,S3C_GPB5_OUTP);  
} 
...
/*
   int temp;
   temp= __raw_readl(S3C_GPBDAT);
   temp=((temp&0x7fff)|1<<15);  0111|1<<15,GPE15 1
   __raw_writel(temp,S3C_GPBDAT);
*/
  s3c_gpio_setpin(S3C_GPB6,1);  
}
...
/*   
   int temp;
   temp= __raw_readl(S3C_GPBDAT);
  temp=temp&0x7fff;              GPE15 0
  __raw_writel(temp,S3C_GPBDAT);
*/
   s3c_gpio_setpin(S3C_GPB6,0);  
}
...
/*
   int temp;
   temp= __raw_readl(S3C_GPBDAT);
  temp=((temp&0xbfff)|1<<14);    1011 GPE14 1
  __raw_writel(temp,S3C_GPBDAT);
*/
  s3c_gpio_setpin(S3C_GPB5,1);  
}    
...
*/
   ...
   temp2=s3c_gpio_getpin(S3C_GPB6);
   temp1=(char)(temp2);
   udelay(10);
   return temp1;
}
...
static struct file_operations s3c_sth11_fops = {
    .owner  =   THIS_MODULE,    
    .open   =   sth11_open,     
    .ioctl  =   sth11_ioctl,
    .read   =   sth11_read,
...
{
	int ret_val=0;
	ret_val=register_chrdev(SHT11_MAJOR,DEVICE_NAME,&s3c_sth11_fops);
	if (ret_val < 0) 
	{
		printk("can't get major %d", SHT11_MAJOR);
		return ret_val;
    	}
